...
PlannerPioneerP3DX::PlannerPioneerP3DX(Robot &robot_, Connection &connection_, std::vector<AnalyzePioneerP3DX> &analyzes_)
	: robot(robot_), connection(connection_), analyzes(analyzes_), no_detection_dist(0.5),
...
//	Temporary
	...
	while (connection.isConnected()) // enquanto a simulação estiver ativa
	{
		for (int i = 0; i < 16; i++)
		{
			char state;
...
//		std::cout << std::endl;
		...
		simxSetJointTargetVelocity(connection.getClientId(), leftMotorHandle, (simxFloat) v_left, simx_opmode_streaming);
