...
PioneerP3DXModel::PioneerP3DXModel(Knowledge& knowledge_)
	: RobotModel("Pioneer_p3dx", knowledge_), robot_handle{-1}, connection{}, robot_position{}, sonars{}
{
	verifyConnection();
	connectToRobot();
}
...
void PioneerP3DXModel::connectToWheels()
{
	...
	wheels.push_back(new WheelVREP{left_wheel_name, knowledge});

	std::string right_wheel_name = "Pioneer_p3dx_rightMotor";
	wheels.push_back(new WheelVREP{right_wheel_name, knowledge});
}
