...
{  // use this when reading data from a script from inside of a custom script function call
	...
	for (int i = 0; i < argCnt; i++)
	{
		...
		if (!done)
		{
			...
			bool expectingATable =
							(((expectedArguments[1 + i * 2 + 0] | SIM_SCRIPT_ARG_NULL_ALLOWED) - SIM_SCRIPT_ARG_NULL_ALLOWED) &
							 sim_script_arg_table) != 0;
			...
			int expectingType =
							(((expectedArguments[1 + i * 2 + 0] | SIM_SCRIPT_ARG_NULL_ALLOWED) - SIM_SCRIPT_ARG_NULL_ALLOWED) |
							 sim_script_arg_table) - sim_script_arg_table;
			if (expectingATable)
			{ // we have a table
				int infoType = 0;
